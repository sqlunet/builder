...
-- ALTER TABLE `@{syntagms.table}` ADD CONSTRAINT `fk_@{syntagms.table}_@{syntagms.sensekey1}` FOREIGN KEY (${syntagms.sensekey1}) REFERENCES ${wnsenses.table} (${wnsenses.sensekey});
-- ALTER TABLE `@{syntagms.table}` ADD CONSTRAINT `fk_@{syntagms.table}_@{syntagms.sensekey2}` FOREIGN KEY (${syntagms.sensekey2}) REFERENCES ${wnsenses.table} (${wnsenses.sensekey});
