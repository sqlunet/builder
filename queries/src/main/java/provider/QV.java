	...
	public String[] query(String key)
	{
		...
		switch (key)
		{
			...
			case "SEMRELATIONS_SYNSETS_WORDS_X_BY_SYNSET":
				...
				actualProjection = Main.appendProjection(actualProjection, String.format("GROUP_CONCAT(%s.%s, ', ' ) AS %s", Words.TABLE, Words.WORD, SemRelations_Synsets_Words_X.MEMBERS2));
			...
			case "LEXRELATIONS_SENSES_WORDS_X_BY_SYNSET":
				...
				actualProjection = Main.appendProjection(actualProjection, String.format("GROUP_CONCAT(DISTINCT %s.%s) AS %s", C.AS_WORD2, Words.WORD, LexRelations_Senses_Words_X.MEMBERS2));
				groupBy = String.format("%s.%s", C.AS_DEST, Synsets.SYNSETID);
				break;
