...
open class BncProcessor(@JvmField protected val conf: Properties) : Processor("bnc") {
    ...
    override fun run() {
        ...
        PrintStream(FileOutputStream(File(outDir, names.file("bncs"))), true, StandardCharsets.UTF_8).use {
            it.println("-- $header")
            processBNCFile(it, File(bncHome, bNCMain), names.table("bncs"), names.columns("bncs"), ThrowingBiConsumer { record: BncRecord, i: Int -> insertRow(it, i.toLong(), record.dataRow()) })
        }
        // subfiles
        ...
        PrintStream(FileOutputStream(File(outDir, names.file("spwrs"))), true, StandardCharsets.UTF_8).use {
            it.println("-- $header")
            processBNCSubFile(it, File(bncHome, bNCSpWr), names.table("spwrs"), names.columns("spwrs"), ThrowingBiConsumer { record: BncRecord, i: Int -> insertRow(it, i.toLong(), record.dataRow()) })
        }
        ...
        PrintStream(FileOutputStream(File(outDir, names.file("convtasks"))), true, StandardCharsets.UTF_8).use {
            it.println("-- $header")
            processBNCSubFile(it, File(bncHome, bNCConvTask), names.table("convtasks"), names.columns("convtasks"), ThrowingBiConsumer { record: BncRecord, i: Int -> insertRow(it, i.toLong(), record.dataRow()) })
        }
        ...
        PrintStream(FileOutputStream(File(outDir, names.file("imaginfs"))), true, StandardCharsets.UTF_8).use {
            it.println("-- $header")
            processBNCSubFile(it, File(bncHome, bNCImagInf), names.table("imaginfs"), names.columns("imaginfs"), ThrowingBiConsumer { record: BncRecord, i: Int -> insertRow(it, i.toLong(), record.dataRow()) })
        }
    }
