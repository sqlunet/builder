...
class BncUpdatingProcessor(conf: Properties) : BncResolvingProcessor(conf) {
    ...
    override fun run() {
        ...
        PrintStream(FileOutputStream(File(outDir, names.updateFile("bncs"))), true, StandardCharsets.UTF_8).use {
            it.println("-- $header")
            processBNCFile(it, File(bncHome, bNCMain), ThrowingBiConsumer { record: BncRecord, i: Int -> updateRow(it, names.table("bncs"), i, record, names.column("bncs.wordid"), names.column("bncs.word")) })
        }
        // subfiles
        ...
        PrintStream(FileOutputStream(File(outDir, names.updateFile("spwrs"))), true, StandardCharsets.UTF_8).use {
            it.println("-- $header")
            processBNCSubFile(it, File(bncHome, bNCSpWr), ThrowingBiConsumer { record: BncRecord, i: Int -> updateRow(it, names.table("spwrs"), i, record, names.column("spwrs.wordid"), names.column("spwrs.word")) })
        }
        ...
        PrintStream(FileOutputStream(File(outDir, names.updateFile("convtasks"))), true, StandardCharsets.UTF_8).use {
            it.println("-- $header")
            processBNCSubFile(it, File(bncHome, bNCConvTask), ThrowingBiConsumer { record: BncRecord, i: Int -> updateRow(it, names.table("convtasks"), i, record, names.column("convtasks.wordid"), names.column("convtasks.word")) })
        }
        ...
        PrintStream(FileOutputStream(File(outDir, names.updateFile("imaginfs"))), true, StandardCharsets.UTF_8).use {
            it.println("-- $header")
            processBNCSubFile(it, File(bncHome, bNCImagInf), ThrowingBiConsumer { record: BncRecord, i: Int -> updateRow(it, names.table("imaginfs"), i, record, names.column("imaginfs.wordid"), names.column("imaginfs.word")) })
        }
    }
