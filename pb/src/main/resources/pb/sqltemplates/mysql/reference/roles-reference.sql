...
ALTER TABLE ${roles.table} ADD CONSTRAINT `fk_@{roles.table}_@{roles.vnroleid}` FOREIGN KEY (${roles.vnroleid}) REFERENCES ${vnroles.table} (${vnroles.roleid});
ALTER TABLE ${roles.table} ADD CONSTRAINT `fk_@{roles.table}_@{roles.fnfeid}` FOREIGN KEY (${roles.fnfeid}) REFERENCES ${fnfes.table} (${fnfes.feid});
